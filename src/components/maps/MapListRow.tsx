  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <div
          className={`w-2 h-2 rounded-full ${isCurrent ? 'bg-cs-yellow animate-pulse' : 'bg-cs-muted/30'}`}
        />
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex gap-1">
              <span
                className={`text-[8px] font-bold px-1.5 py-0.5 rounded tracking-widest uppercase ${map.type === 'workshop' ? 'bg-cs-blue/20 text-cs-blue border border-cs-blue/30' : 'bg-cs-muted/10 text-cs-muted border border-cs-border'}`}
              >
                {map.type === 'workshop' ? 'Workshop' : 'Default'}
              </span>
              {map.tag && (
                <span
                  className={`text-[8px] font-bold px-1.5 py-0.5 rounded tracking-widest uppercase ${
                    map.tag === 'Defusal'
                      ? 'bg-cs-red/20 text-cs-red border border-cs-red/30'
                      : map.tag === 'Hostage Rescue'
                        ? 'bg-cs-green/20 text-cs-green border border-cs-green/30'
      ...
      <div className="flex items-center gap-3">
        {isCurrent && (
          <span className="text-[9px] font-bold text-cs-yellow uppercase tracking-widest bg-cs-yellow/10 px-2 py-0.5 rounded">
            Current
          </span>
        )}
