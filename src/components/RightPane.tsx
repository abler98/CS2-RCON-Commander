...
export default function RightPane() {
  const { consoleHistory, setConsoleHistory } = useRconContext();
        ...
        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
          {consoleHistory.length > 0 ? (
            <div className="space-y-4">
              {groupConsoleHistory(consoleHistory)
                .reverse()
                .slice(0, 30)
                .map((item, i) => (
                  <div
                    key={i}
                    className={`p-3 rounded border-l-2 text-[10px] font-mono leading-relaxed bg-white/[0.02] shadow-sm ${
                      item.type === 'pair'
                        ? 'border-cs-yellow/50'
                        : item.type === 'command'
                          ? 'border-cs-yellow'
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <span
                        className={`font-bold tracking-widest ${
                          item.type === 'pair' || item.type === 'command'
                            ? 'text-cs-yellow'
                            : item.type === 'error'
                              ? 'text-cs-red'
                              : 'text-cs-muted'
            ...
            <div className="h-full flex flex-col items-center justify-center text-cs-muted/10 opacity-50 space-y-4">
              ...
              <p className="text-[9px] uppercase tracking-widest text-center">
                Awaiting System
                <br />
                Logs
              </p>
