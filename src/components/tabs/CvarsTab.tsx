...
export default function CvarsTab() {
  const { setShowRawModal, cvars, isLoadingCvars, fetchCvars, updateCvar } = useCvarsContext();
  ...
  const filteredCvars = cvars.filter(
    (c) =>
      c.name.toLowerCase().includes(cvarSearch.toLowerCase()) ||
      c.description.toLowerCase().includes(cvarSearch.toLowerCase()),
  );

  const paginatedCvars = filteredCvars.slice(
    (cvarPage - 1) * CVARS_PER_PAGE,
    cvarPage * CVARS_PER_PAGE,
  );
      ...
      <div className="flex-1 bg-cs-bg-panel border border-cs-border rounded-lg overflow-hidden flex flex-col">
        <div className="overflow-y-auto flex-1 custom-scrollbar">
          <table className="w-full text-left">
            <thead className="sticky top-0 bg-cs-bg-main z-10 border-b border-cs-border">
              <tr>
                <th className="px-6 py-4 text-[10px] font-bold text-cs-muted uppercase tracking-widest">
                  Variable Name
                </th>
                <th className="px-6 py-4 text-[10px] font-bold text-cs-muted uppercase tracking-widest">
                  Current Value
            ...
            <tbody className="divide-y divide-cs-border/30">
              {isLoadingCvars && (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-12 text-center text-cs-muted font-mono animate-pulse"
                  >
              ...
              {!isLoadingCvars &&
                paginatedCvars.map((cvar) => (
                  <tr key={cvar.name} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-6 py-4">
                      <span className="font-mono text-xs font-bold text-cs-yellow">
                        {cvar.name}
                      </span>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="px-6 py-4 border-t border-cs-border bg-cs-bg-main flex items-center justify-between shrink-0">
          <div className="text-[10px] font-bold text-cs-muted uppercase tracking-widest">
            Showing {(cvarPage - 1) * CVARS_PER_PAGE + 1} to{' '}
            {Math.min(cvarPage * CVARS_PER_PAGE, filteredCvars.length)} of {filteredCvars.length}
          </div>
          <div className="flex items-center gap-4">
