      ...
      >
        ...
        <div className="p-8">
          <div className="flex items-center gap-4 mb-6">
            <div
              className={`p-3 rounded-lg ${confirmModal.type === 'map' ? 'bg-cs-yellow/20' : 'bg-cs-blue/20'}`}
            >
              {confirmModal.type === 'map' ? (
                <MapIcon className="w-6 h-6 text-cs-yellow" />
              ) : (
                <Zap className="w-6 h-6 text-cs-blue" />
              )}
            </div>
            <div>
              <h3 className="text-xl font-bold tracking-tight">{confirmModal.title}</h3>
              <p className="text-xs text-cs-muted font-bold uppercase tracking-widest mt-1">
          ...
          <p className="text-sm text-cs-muted leading-relaxed mb-8">{confirmModal.description}</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`flex-1 px-6 py-3 rounded-lg text-sm font-bold transition-all shadow-lg shadow-black/20 ${
                confirmModal.type === 'map'
                  ? 'bg-cs-yellow text-black hover:scale-[1.02]'
                  : 'bg-cs-blue text-white hover:scale-[1.02]'
              }`}
            >
              Confirm Change
            </button>
