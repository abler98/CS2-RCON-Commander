...
export default function RawCvarsModal() {
  const { rawCvars, setShowRawModal } = useCvarsContext();
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      >
        <div className="p-6 border-b border-cs-border flex items-center justify-between">
          ...
          <button
            onClick={() => setShowRawModal(false)}
            className="p-1 hover:bg-white/5 rounded text-cs-muted hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar font-mono text-[11px] leading-relaxed text-cs-muted bg-cs-bg-console">
          <pre className="whitespace-pre-wrap">
            {rawCvars ||
              'No raw data available. Ensure connection is active and data has been fetched.'}
          </pre>
        </div>
      </motion.div>
    </div>
