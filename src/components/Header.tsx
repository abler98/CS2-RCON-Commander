...
export default function Header() {
  const { config, isConnected, isConnecting, setShowConfig, executeCommand, resetConfig } =
    useRconContext();
    ...
    <header className="h-16 border-b border-cs-border bg-cs-bg-panel flex items-center justify-between px-6 shrink-0 z-50">
      ...
      <div className="flex items-center gap-8">
        <div className="flex flex-col items-end">
          ...
          <span
            className={`text-xs flex items-center gap-1.5 ${isConnected ? 'text-cs-green' : 'text-cs-red'}`}
          >
            <span
              className={`w-2 h-2 rounded-full ${isConnected ? 'bg-cs-green animate-pulse' : 'bg-cs-red'}`}
            ></span>
        ...
        <div className="hidden md:flex flex-col items-end">
          ...
          <div className="h-4 flex items-center gap-1.5 min-w-[60px] justify-end">
            {serverInfo ? (
              <span
                className={`text-xs font-medium ${isConnected && serverInfo ? 'text-cs-blue' : 'text-cs-muted'}`}
              >
            ...
            {serverInfo ? (
              <>
                <div className="flex items-center gap-1.5 justify-end">
                  <span
                    className={`text-xs font-bold tracking-tight ${isConnected && serverInfo ? 'text-cs-yellow' : 'text-cs-muted'}`}
                  >
                    {MAP_LIST.find((m) => m.id === serverInfo.map)?.name ||
                      workshopMaps.find((m) => m.id === serverInfo.map || m.name === serverInfo.map)
                        ?.name ||
                      (serverInfo.map &&
        ...
        <div className="hidden sm:flex flex-col items-end">
          ...
          <div className="h-4 flex items-center gap-1.5 min-w-[50px] justify-end">
            {serverInfo ? (
              <span
                className={`text-xs font-medium ${isConnected && serverInfo ? 'text-cs-text' : 'text-cs-muted'}`}
              >
        ...
        <div className="hidden lg:flex items-center gap-6 border-l border-cs-border pl-6 ml-2">
          <div className="flex flex-col items-end">
            ...
            <span
              className={`text-xs font-mono font-bold ${isConnected ? 'text-cs-blue' : 'text-cs-muted'}`}
            >
              {isConnected ? '99.1%' : '--'}
            </span>
          </div>
          <div className="flex flex-col items-end">
            ...
            <span
              className={`text-xs font-mono font-bold ${isConnected ? 'text-cs-green' : 'text-cs-muted'}`}
            >
              {isConnected ? '128.0' : '--'}
            </span>
          </div>
          <div className="flex flex-col items-end">
            ...
            <span
              className={`text-xs font-mono font-bold ${isConnected ? 'text-cs-text' : 'text-cs-muted'}`}
            >
              {isConnected ? '0.008' : '--'}
            </span>
          </div>
